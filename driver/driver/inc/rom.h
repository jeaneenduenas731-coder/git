...
typedef enum _UPGRADE_ERRNO{
	...
	ERRNO_CHIP_INFO_ERR, 		//3  芯片型号错误
	ERRNO_FLASH_DRV_ERR, 		//4 FLASH DRV 错误，例如FlashDrv的size或者校验错误
	ERRNO_CMD_ERR, 	            //5 Command解析错误
	ERRNO_CODE_ERR,             //6 Code解析错误
	ERRNO_DATA_ERR,	            //7 Data解析错误
    ERRNO_FLASH_ERR,            //8 FLASH 擦写错误
	ERRNO_UPGRADED_CRC_ERR, 	//9    升级CODE后CRC校验错误
	ERRNO_KEY_MISMATCH_ERR,		//10 key 不一致性错误
	ERRNO_BANKA_CRC_ERR, 	    //11 BANK A crc校验错误
	STATE_NEED_UPGRADE,         //12   新升级，外部配置
