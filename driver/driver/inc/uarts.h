...
typedef enum
{
    ...
    //set to 1 to enable rx overtime function.
    UART_IOCTL_OVERTIME_SET,    
    //set to 1 to clear the overtime flag.
 ...
 */
bool UART_Init(UART_PORT_T UART_MODULE, uint32_t BaudRate, uint8_t DataBits, uint8_t Parity, uint8_t StopBits);
 ...
 * @return     接收到的数据长度
 */
uint32_t UART_Recv(UART_PORT_T UART_MODULE, uint8_t *RecvBuf, uint32_t BufLen, uint32_t TimeOut);
 ...
 */
bool UART_RecvByte(UART_PORT_T UART_MODULE, uint8_t *Val);
 ...
 * @return     发送长度
 */
uint32_t UART_Send(UART_PORT_T UART_MODULE, uint8_t* SendBuf, uint32_t BufLen,uint32_t TimeOut);
 ...
 * @return     无返回值
 */
void UART_SendByte(UART_PORT_T UART_MODULE, uint8_t SendByte);
 ...
 * @return     无返回值
 */
void UART_SendByte_In_Interrupt(UART_PORT_T UART_MODULE, uint8_t SendByte);
 ...
 */
int32_t UART_IOCtl(UART_PORT_T UART_MODULE, UART_IOCTL_CMD_T Cmd, uint32_t Arg);
