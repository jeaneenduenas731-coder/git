...
#if defined(CFG_APP_HDMIIN_MODE_EN)
bool Hdmi_Int_flag = 0;
#endif
...
{
	KeyScanMsg CodeKey = {KEY_MSG_INDEX_EMPTY, KEY_UNKOWN_TYPE,CODE_KEY_SOURCE};

	if(ClockWiseCnt)
	{
		CodeKey.index = 0;
		CodeKey.type = KEY_RELEASED;
	}
...
{
#if defined(CFG_APP_HDMIIN_MODE_EN)
	extern bool HDMI_HPD_INTFlagGet(void);
	extern void HDMI_HPD_INTFlagClear(void);

	if(HDMI_HPD_INTFlagGet())
	{
