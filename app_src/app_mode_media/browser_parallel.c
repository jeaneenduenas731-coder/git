...
void BrowserUp(BrowserStep BrowserProcessTemp)
 {
    ...
    if(BrowserProcessTemp==Browser_Folder)
    {
            ...
            else
            {
                ...
                {
	                if(gMediaPlayer->ValidFolderSumInDisk>=GUI_ROW_CNT_MAX)
	                {
		                gFolderFocusing=GUI_ROW_CNT_MAX;
		                gStartFolderIndex=gMediaPlayer->ValidFolderSumInDisk-GUI_ROW_CNT_MAX+1;
	                }
					else
					{
						gStartFolderIndex=1;
						gFolderFocusing=gMediaPlayer->ValidFolderSumInDisk;
					}
                }
				
				APP_DBG("UP gpMediaPlayer->ValidFolderSumInDisk=%d\n",gMediaPlayer->ValidFolderSumInDisk);
            }
    ...
    if(BrowserProcessTemp==Browser_Folder)
        {
            ...
            {// full display the last time
                ...
                {
                    public_temp_var=1;//out of range ,need scan again
                    if(gStartFolderIndex+GUI_ROW_CNT_MAX-1<gMediaPlayer->ValidFolderSumInDisk)// bkd
                    {
                        gStartFolderIndex++;
                    }
                    else
    ...
    case Browser_Folder:
    	...
    	switch(msgId)
        	...
        	case MSG_MEDIA_PLAY_BROWER_ENTER:
        		...
        		gMediaPlayer->ErrFileCount=gBrowserDirWin[gFolderFocusing - 1].valid_file_num;
