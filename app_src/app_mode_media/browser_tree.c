...
static bool brows_enter(brows_tree_cnxt *cnxt)
{
	...
	else if(p->tree[p->ptr_num].type == ENTRY_TYPE_FILE)// here exit browser after play song
	{
		...
		if(f_get_file_num_by_path_chain(cnxt->volume, path_chain, chain_len, f_hdr, &file_num) == FR_OK)
		{
			...
			gMediaPlayer->CurFileIndex=file_num;
...
static void EnterBrowserPlayMode(void)
{
	...
	if(g_brows_cnxt!=NULL)
		...
		{
		APP_DBG("browser memory not enough %d %d\n",sizeof(brows_tree_cnxt),osPortRemainMem());
		return;
		}
...
{
	memset(&StoryInformation[0],0,(sizeof(StoryFolderInfo))*FOLDER_TYPE_MAX);
	FolderType=ERGE;
	...
	StoryInformation[GUOXUE].PlayTime =0;
	...
	StoryInformation[YINGYU].PlayTime =0;
...
void FindFileHdrByDir(uint32_t Array_index)// Array_index-0:Current ,1:next
{
        ...
        switch(FolderType)
        {
			...
			memcpy(&gMediaPlayer->PlayerFile,&file_win.file_hdr,sizeof(FIL));
...
void DecSongNumber(void)
{
	if(StoryInformation[FolderType].IndexInFolder<=1)
		...
		StoryInformation[FolderType].IndexInFolder--;
	StoryInformation[FolderType].PlayTime = 0;
}

void AddSubFoldersNumber(bool dir)
{
    if(dir)
        FolderType = (FolderType + 1) % FOLDER_TYPE_MAX;
    else
        FolderType = (FolderType + FOLDER_TYPE_MAX -1) % FOLDER_TYPE_MAX;
}

void SetCurFoldersSongPlayTime(uint32_t PlayTime)
{
	StoryInformation[FolderType].PlayTime = PlayTime;
}

uint32_t GetCurFoldersSongPlayTime(void)
{
	return StoryInformation[FolderType].PlayTime;
}

void SaveStoryVar(void *buf)
{
	StoryFolderSaveInfo *p = (StoryFolderSaveInfo *)buf;
	uint8_t i;

	for(i=0; i<FOLDER_TYPE_MAX;i++)
	{
		p->IndexInFolder[i] 	= StoryInformation[i].IndexInFolder;
		p->OffsetInFolder[i][0] = StoryInformation[i].OffsetInFolder[0];
		p->OffsetInFolder[i][1]	= StoryInformation[i].OffsetInFolder[1];
		p->PlayTime[i] 			= StoryInformation[i].PlayTime;
	}
	p->FolderType = FolderType;
}

void LoadStoryVar(void *buf)
{
	StoryFolderSaveInfo *p = (StoryFolderSaveInfo *)buf;
	uint8_t i;
	...
	for(i=0; i<FOLDER_TYPE_MAX;i++)
	{
		StoryInformation[i].IndexInFolder 		= p->IndexInFolder[i] ;
		StoryInformation[i].OffsetInFolder[0]	= p->OffsetInFolder[i][0] ;
		StoryInformation[i].OffsetInFolder[1]	= p->OffsetInFolder[i][1];
		StoryInformation[i].PlayTime			= p->PlayTime[i];
	}
