...
void UsbLoadBQBCmd(uint16_t len,uint8_t *buff)
{
	uint16_t position;
	position = 0;
	...
	while(position < len)
	{
		if((buff[position] == 'B') && (buff[position+1] == 'T') && (buff[position+2] == '+'))
		{
...
//	APP_DBG("bt_bqb_cmd_process\n");
	...
	switch(i)
	{
		...
		case BT_BQB_CMD_CONNECT_CTRL:
			...
			msgSend.msgId = MSG_BT_CONNECT_CTRL;
		break;

		case BT_BQB_CMD_CLEAR_PAIRED:
			APP_DBG("BQB: BT_BQB_CMD_CLEAR_PAIRED\n");
			msgSend.msgId = MSG_BT_CLEAR_PAIRED_LIST;
		break;
		...
		case BT_BQB_CMD_HANGUP:
			BtStackServiceMsgSend(MSG_BTSTACK_MSG_BT_HANGUP);
			return;
			break;

		case BT_BQB_CMD_HF_VRR_BV_01_C:
			APP_DBG("BQB: BT_BQB_CMD_HF_VRR_BV_01_C\n");
			msgSend.msgId = MSG_BT_BQB_VRR_BV_01_C;
		break;
