...
#define SBC_SUBBAND 8

#define ENC_BITPOOL		ENCODE_QUALITY
#define DEC_BITPOOL 	DECODE_QUALITY

#if defined(ENCODE_CH) && ENCODE_CH == 2 && defined(AUDIO_LOWFRAME)
	#define SBC_ENC_LEN_PER_FREME (4+(4 * SBC_SUBBAND * 2 )/8 + ((SBC_SUBBAND + SBC_BLOCK * ENC_BITPOOL) / 8))
#elif defined(ENCODE_CH) && ENCODE_CH != 0
	#define SBC_ENC_LEN_PER_FREME (4+(4 * SBC_SUBBAND * ENCODE_CH )/8 + ((SBC_BLOCK * ENCODE_CH * ENC_BITPOOL) / 8))
#endif

#if defined(DECODE_CH) && DECODE_CH == 2 && defined(AUDIO_LOWFRAME)
	#define SBC_DEC_LEN_PER_FREME (4+(4 * SBC_SUBBAND * 2 )/8 + ((SBC_SUBBAND + SBC_BLOCK * DEC_BITPOOL) / 8))
#elif defined(DECODE_CH) && DECODE_CH != 0
	#define SBC_DEC_LEN_PER_FREME (4+(4 * SBC_SUBBAND * DECODE_CH )/8 + ((SBC_BLOCK * DECODE_CH * DEC_BITPOOL) / 8))
#endif

#ifdef SBC_ENC_LEN_PER_FREME
	#define RFAUDIO_FRAME_LEN		SBC_ENC_LEN_PER_FREME
#else
	#define RFAUDIO_FRAME_LEN		SBC_DEC_LEN_PER_FREME
#endif
//wireless payload/tx fifo packet Class A¡¢B¡¢C
#define RFAUDIO_TRANS_A				((RFAUDIO_FRAME_LEN - CRC_PACKSUB + PACKET_CRC_LEN + PACKET_CNT_LEN) * RFPACK_NAUDIO)
#define RFAUDIO_TRANS_B				((RFAUDIO_FRAME_LEN) * RFPACK_NAUDIO + PACKET_CRC_LEN + PACKET_CNT_LEN)
#define RFAUDIO_TRANS_C				(((RFAUDIO_FRAME_LEN - CRC_PACKSUB) * 3) / 2 + PACKET_CRC_LEN * 2 + PACKET_CNT_LEN) //1.5Frame
#define RFAUDIO_TRANS_D				((RFAUDIO_FRAME_LEN - CRC_PACKSUB) * RFPACK_NAUDIO + PACKET_CRC_LEN + PACKET_CNT_LEN)
#define RFAUDIO_TRANS_E				((RFAUDIO_FRAME_LEN - CRC_PACKSUB+PACKET_CRC_LEN) * RFPACK_NAUDIO + PACKET_CNT_LEN)
#define RFAUDIO_TRANS_F				((RFAUDIO_FRAME_LEN - CRC_PACKSUB) * RFPACK_NAUDIO + PACKET_CNT_LEN)
#define	RFAUDIO_LEN_PER_FREME		(RFAUDIO_FRAME_LEN * RFPACK_NAUDIO)
...
#define SBC_MAX_NUM_OF_CHANNELS 1
...
#define SBC_JOINT_STE_INCLUDED FALSE
#endif
