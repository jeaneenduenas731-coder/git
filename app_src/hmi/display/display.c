...
void DispRepeat(void)
{
	...
	char RepeatName[5][4] =
	{
		...
		if(gMediaPlayer)
			DispString(&RepeatName[gMediaPlayer->CurPlayMode][0]);
		DispIcon(ICON_DP_FM_MHZ, LIGHT_OFF);
		DispIcon(ICON_COLON, LIGHT_OFF);
...
#if (defined(CFG_APP_CARD_PLAY_MODE_EN) )|| defined(CFG_APP_USB_PLAY_MODE_EN)
	//APP_DBG((">>DispFileNum\n"));
	if(gMediaPlayer && gMediaPlayer->CurFileIndex > 0)	//if(gFsInitOK == 1)
	{
		//APP_DBG(("gPlayCtrl.FileNum:%u\n", gPlayCtrl.FileNum));
		Num2Char(gMediaPlayer->CurFileIndex, FALSE);
	}
	else
	{
		//APP_DBG(("NOF\n"));
...
#if (defined(CFG_APP_CARD_PLAY_MODE_EN) )|| defined(CFG_APP_USB_PLAY_MODE_EN)
	WORD TimeMMSS;
	if(gMediaPlayer == NULL)return;
	TimeMMSS = gMediaPlayer->CurPlayTime / 60;
	TimeMMSS *= 100;
	TimeMMSS += gMediaPlayer->CurPlayTime % 60;
...
{	
		...
		switch(GetSystemMode())
		{
			...
			case ModeUDiskAudioPlay:
			case ModeCardAudioPlay:
				break;
#endif
			...
			case ModeRadioAudioPlay:
				if((gRadioControl->Freq >=875) && (gRadioControl->Freq <=1080))
				 {
				  DispIcon(ICON_USB, LIGHT_OFF);
				  DispIcon(ICON_SD, LIGHT_OFF);
				  ...
				  DispIcon(ICON_PAUSE, LIGHT_OFF);
				  DispIcon(ICON_COLON, LIGHT_OFF);
				  Num2Char(gRadioControl->Freq, FALSE);
				 }
				/*
...
void Display(uint16_t msgRecv)
{
		...
		switch(msgRecv)
		{
		   ...
		   case MSG_DISPLAY_SERVICE_SEARCH_STATION:
			     DispRadioStation(gRadioControl->ChlCount + 1);
			   break;

		   case MSG_DISPLAY_SERVICE_STATION:
			     DispRadioStation(gRadioControl->CurStaIdx + 1);
			   break;
#endif
