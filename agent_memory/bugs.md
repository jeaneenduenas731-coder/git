...
- 用户要求灯效模式一的灯珠颜色全部改成蓝色，已按最小范围修改 `LedEffect1()` 的点亮颜色逻辑。

## 已知风险

- 当前修改未在实机观察实际视觉效果，需要在设备上确认模式一 LED_DATA_3/LED_DATA_2 都符合预期蓝色显示。
...
- 已将 `LedEffect1()` 中 LED_DATA_3 和 LED_DATA_2 的固定红色显示改成统一蓝色显示。
